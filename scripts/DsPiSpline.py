...
#This script calculates the coefficients a, b, c, d in acc(t) = a*t^3 + b*t^2 + c*t + d for each bin of a natural spl\|Warning (initialization): An error occurred while loading                                                                                                                
#Since for the phi_s measurement the spline ends before the right edge, the region above it is modeled with a linear  
#interpolation from the last spline bin.                                                                              
...
def linear_interpolation(spline_knots, spline_coeff, knot_position="last"):
    ...
    if(knot_position=="first"):
        ...
        sympy.pprint("Error: Wrong knot position. Exiting...")
        return
...
def print_spline(spline_knots, spline_coeff, interpolate = 0):
    t = sympy.Symbol('t')
    M = sympy.zeros(len(spline_knots)-1,4)
    for bin_i in range(0,len(spline_knots)-1):
        a = S(t, get_knot(spline_knots, bin_i-2), get_knot(spline_knots, bin_i-1), get_knot(spline_knots, bin_i), get_knot(spline_knots, bin_i+1), get_knot(spline_knots, bin_i+2), get_knot(spline_knots, bin_i+3), spline_coeff[bin_i], spline_coeff[bin_i+1], spline_coeff[bin_i+2], spline_coeff[bin_i+3]        )                
        b = a.expand().as_coefficients_dict() ## t3,t2,t,                                                             |
        for j in range(4): 
            M[bin_i,j] = b[t**j]          
...
# print(binpars_2017)
# print(binpars_2018)
# coefs_ram = [0.209, 0.354, 0.741, 0.920, 1.158, 1.363, 1.0, 0.879]
# binpars_2018 = print_spline(real_knots, coefs_ram, interpolate = 1)
# for i in range(1, len(knots)-1):
#     x = np.linspace(knots[i], knots[i+1],50)
#     y = binpars_2018[i][3]*x**3 + binpars_2018[i][2]*x**2 + binpars_2018[i][1]*x + binpars_2018[i][0]
#     plt.plot(x,y, 'r')
# plt.savefig('hipercaca.pdf')
# exit()
